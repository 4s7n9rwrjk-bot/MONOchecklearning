...
function getCfg(){
  return {
    url:SUPA_URL_KEYS.map(k=>window[k]).find(Boolean)||'',
    key:SUPA_KEY_KEYS.map(k=>window[k]).find(Boolean)||''
  };
}
function msg(t){
  let e=document.getElementById('syncMessage');
...
function status(t){
  ...
  if(d)d.textContent=user?'●':'○';
}
function localStamp(){
  return Number(localStorage.getItem('monocheck-study-updated')||0);
}
...
async function ensureClient(){
  ...
  initPromise=(async()=>{
    ...
    if(!cfg()){
      throw new Error('Supabase設定が読み込めていません。supabase-config.js のURLとPublishable keyを確認してください。');
    }
    ...
    client=window.supabase.createClient(c.url,c.key,{
      auth:{
        ...
        storageKey:'monocheck-study-auth',
        flowType:'pkce'
      }
    });
...
window.openSyncModal=function(){
  let m=document.getElementById('syncModal');
  if(m)m.style.display='flex';

  if(!cfg()){
    status('Supabase未設定：端末内保存のみ');
    msg('supabase-config.js のURLとPublishable keyを確認してください。');
  }else if(user){
    status('同期アカウントに接続済み');
  }else{
    status('ログインしてください');
...
window.syncSignUp=async function(){
  try{
    ...
    if(!email||password.length<6){
      return msg('メールアドレスと6文字以上のパスワードを入力してください');
    }
...
window.syncNow=async function(){
  if(!client||!user){
    return msg('Supabaseにログインしてください');
  }
  ...
  try{
    let {data:r,error:e}=await client
      .from('monocheck_study_data')
...
window.queueStudyCloudSync=function(){
  if(!user){
    return;
  }
